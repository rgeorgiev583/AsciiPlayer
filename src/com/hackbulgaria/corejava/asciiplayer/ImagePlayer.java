...
public class ImagePlayer extends FilePlayer {
    ...
    public ImagePlayer(BufferedImage image) {
        this.image = image;
    }
    ...
    public String getMedia() {
        ...
        for (int i = 0; i < height; i += blockHeight) {
            for (int j = 0; j < width; j += blockWidth) {
                art += getAsciiPixel(j, i, j + blockWidth < width ? blockWidth : width - j,
                        i + blockHeight < height ? blockHeight : height - i);
            }
    ...
    public void play() {
        ...
        for (int i = 0; i < height; i += blockHeight) {
            for (int j = 0; j < width; j += blockWidth) {
                System.out.print(getAsciiPixel(j, i, j + blockWidth < width ? blockWidth : width - j,
                        i + blockHeight < height ? blockHeight : height - i));
            }
